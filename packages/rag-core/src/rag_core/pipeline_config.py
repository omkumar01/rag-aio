"""Declarative pipeline configuration shared across the rag-aio platform.
...
It lives in ``rag-core`` (not ``rag-orchestrator``) so that the ``rag-aio``
facade can validate pipeline configuration with only its dependency-light base
install; component construction/compilation from a config happens in
